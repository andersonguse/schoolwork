...
import java.sql.*;
@WebServlet(name = "EditPatientInfoServlet", urlPatterns = {"/EditPatientInfoServlet"})
public class EditPatientInfoServlet extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try{
            p1.updateDB(fname, lname, address, email, insuranceco);
            p1.selectDB(p1.getID());
            }
            catch(SQLException | ClassNotFoundException ex){}
            session.setAttribute("p1", p1);
            response.sendRedirect("editInfoPatient.jsp");
        }
    }
